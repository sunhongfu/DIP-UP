...
class Unet_1Chan_9Class(nn.Module):
    def __init__(self, EncodingDepth):
        super(Unet_1Chan_9Class, self).__init__()
        self.EncodeConvs = []
        self.DecodeConvs = []
...
################### Encoding Layers #######################
        for encodingLayer in temp:
            if encodingLayer == 1:
                ...
                self.EncodeConvs.append(EncodingBlocks(1, num_outputs))  # ZX edit
                # Remember When doing the inference, Change the Input-Channel Number, e.g. Unet_1ChanIn()
