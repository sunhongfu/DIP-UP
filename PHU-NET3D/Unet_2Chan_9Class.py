...
class Unet_2Chan_9Class(nn.Module):
    def __init__(self, EncodingDepth):
        super(Unet_2Chan_9Class, self).__init__()
        self.EncodeConvs = []
        self.DecodeConvs = []
        self.EncodingDepth = EncodingDepth
        # initial_num_layers = 32  # XYZ edit
...
################### Encoding Layers #######################
        for encodingLayer in temp:
            if encodingLayer == 1:
                ...
                self.EncodeConvs.append(EncodingBlocks(2, num_outputs))  # ZX edit
                # Remember When doing the inference, Change the Input-Channel Number, e.g. Unet_2ChanIn()
                # self.EncodeConvs.append(EncodingBlocks(1, num_outputs))  ## XYZ old version
            else:             
                num_outputs = initial_num_layers * 2 ** (encodingLayer - 1)
...
################### Decoding Layers #######################
        ...
        # self.FinalConv = nn.Conv3d(num_inputs, 35, 1, stride = 1, padding = 0)
        self.FinalConv = nn.Conv3d(num_inputs, 9, 1, stride=1, padding=0)
        # Single Echo with TE=10ms Relative count number for 8 Classes (Index 0~6 for wrapping counts and Class #7 for BKG) XYZ 03112022 Temp Test
        # Single Echo with TE=10ms Relative count number for 9 Classes (Index 1~8 for wrapping counts and Class #0 for BKG) XYZ 09012023 Temp Test
        # Single Echo with TE=15ms Relative count number for 11 Classes (Index 0~9 for wrapping counts and Class #10 for BKG)
        # Single Echo with TE=15ms Global count number for 14 Classes (Index 1~13 for wrapping counts and Class #0 for BKG)
        # The second dimension (num_inputs, classNum, 1, stride = 1, padding = 0), change to the class number (e.g., 0-39 for 40 classes)
################## End Section 2 ##########################
